      ...
      <template v-for="match in matches" :key="match.id">
        <a class="match" :href="matchUrl(match.id)">
          ...
          <template v-if="match.team1.id == team.id">
            ...
            <div>{{ match.team2.score < match.team1.score ? "Win" : (match.team2.score > match.team1.score ? "Loss" : "Tie")}}</div>
            <div class="left">{{ match.team2.roster }}</div>
            <div class="left"><a :href="teamUrl(match.team2.id)">{{ match.team2.name }}</a></div>
            <div>{{ match.team1.teamValue }}</div>
            <div>{{ match.team1.casualties.bh }} / {{ match.team1.casualties.si }} / {{ match.team1.casualties.rip }}</div>
            <div>{{ match.team2.casualties.bh }} / {{ match.team2.casualties.si }} / {{ match.team2.casualties.rip }}</div>
          </template>
          <template v-else>
            <div>{{ match.team2.score }} - {{ match.team1.score }}</div>
            <div>{{ match.team1.score < match.team2.score ? "Win" : (match.team1.score > match.team2.score ? "Loss" : "Tie")}}</div>
            <div class="left">{{ match.team1.roster }}</div>
            <div class="left"><a :href="teamUrl(match.team1.id)">{{ match.team1.name }}</a></div>
            <div>{{ match.team1.teamValue }}</div>
            <div>{{ match.team2.casualties.bh }} / {{ match.team2.casualties.si }} / {{ match.team2.casualties.rip }}</div>
            <div>{{ match.team1.casualties.bh }} / {{ match.team1.casualties.si }} / {{ match.team1.casualties.rip }}</div>
          </template>
...
class TeamMatches extends Vue {
  ...
  public async loadMatches() {
    ...
    if (apiResponse.isSuccessful()) {
        ...
        this.triggerUnexpectedError('Loading team matches: ' + apiResponse.getErrorMessage());
    }
  }
