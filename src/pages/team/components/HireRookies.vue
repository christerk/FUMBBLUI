<template>
  <div class="hirerookies">
    ...
    <div
      class="hirerookie"
      v-for="positionDataForBuyingPlayer in rosterPositionDataForBuyingPlayer"
      :key="positionDataForBuyingPlayer.positionId"
    >
      <div class="rookieheader">
        <div
          class="positionname"
          :title="positionDataForBuyingPlayer.position.name"
        >
          {{ positionDataForBuyingPlayer.position.name }}
        </div>
        ...
        <div class="positioncost">
          {{ positionDataForBuyingPlayer.position.cost / 1000 }}k
        </div>
      </div>
      <div class="rookiebody">
        <div class="iconwithstats">
            ...
            <div
              ...
              :style="
                rosterIconManager.getIconStyle(
                  positionDataForBuyingPlayer.positionId,
                  null,
                )
              "
            ></div>
          ...
          <table class="rookiestats">
            ...
            <tr>
              ...
              <td>
                {{
                  positionDataForBuyingPlayer.position.stats.Passing
                    ? `${positionDataForBuyingPlayer.position.stats.Passing}+`
                    : "-"
                }}
              </td>
          ...
          <div class="buying">
            <button
              ...
              class="teambutton"
              :class="{
                disabled: !canBuyPosition(positionDataForBuyingPlayer),
              }"
              :title="
                reasonsCannotBuy(positionDataForBuyingPlayer).join('\r\n')
              "
            >
              Hire
            </button>
        ...
        <div class="skillrow">
          <div class="skills">
            {{ positionDataForBuyingPlayer.position.skills.join(", ") }}
          </div>
          <div class="categories">
            <span class="primary">{{
              positionDataForBuyingPlayer.position.primarySkills.join("")
            }}</span>
            <span class="secondary">{{
              positionDataForBuyingPlayer.position.secondarySkills.join("")
            }}</span>
          </div>
        </div>
        <div
          class="quantityprogressouter"
          v-if="positionDataForBuyingPlayer.position.quantityAllowed > 100"
        >
          <div class="quantityprogress">
            <div
              v-for="item in positionDataForBuyingPlayer.position
                .quantityAllowed"
              :key="item"
              :class="{
                yes: item <= positionDataForBuyingPlayer.quantityHired,
                no: item > positionDataForBuyingPlayer.quantityHired,
              }"
            ></div>
          </div>
        </div>
