<template>
    ...
    <div v-if="page == 'squads'">
      <template v-if="squads.length === 0">
        <div class="no-squads-message tcenter">
          ...
          <p>Use the tabs above to create or join a tournament squad.</p>
        </div>
      </template>
      <template v-else v-for="squad in squads" :key="squad.id">
        <TitledPanel id="squads">
          <template #header>
            ...
            <div class="tournament" v-if="squad.tournament != null">
          ...
          <template #content>
            <div
              :class="{
                squad: true,
                allowEdit: isCoachCaptain(squad, coachName),
              }"
            >
              ...
              <div
                class="members"
                @dragover.prevent
                @drop.prevent="onDropMember($event, squad, false)"
              >
                <div
                ...
                >
                  <div class="coach">
                    <span
                      ...
                      >&#9733; </span
                    ><a href="#">{{ member.coach }}</a>
                  </div>
                  <div class="teamname">{{ member.name }}</div>
                  <div class="teamlogo">
                    <img
                      :src="'https://fumbbl.com/i/' + member.logo"
                      :alt="member.roster"
                      :title="member.roster"
                    />
                  </div>
                ...
                <template
                  v-if="squad.limits.maxMembers - squad.members.length > 0"
                >
                  <div
                    class="member empty"
                    @dragover.prevent
                  ...
                  >
                    {{
                      openSpotString(
                        squad.limits.maxMembers - squad.members.length,
                      )
                    }}
                  </div>
                </template>
              ...
              <div v-if="squad.limits.maxReserves > 0">
                ...
                >
                  <div
                    v-for="member in squad.reserves"
                    :key="member.id"
                    :teamid="member.teamId"
                    class="member"
                    draggable="true"
                  ...
                  <template
                    v-if="squad.limits.maxReserves - squad.reserves.length > 0"
                  >
                    <div
                      class="member empty"
                      @dragover.prevent
                    ...
                    >
                      {{
                        openSpotString(
                          squad.limits.maxReserves - squad.reserves.length,
                        )
                      }}
                    </div>
                  </template>
              ...
              <div class="squadfooter">
                <div class="inviteWrap">
                  ...
                  <div
                    class="inviteInfo"
                    v-if="
                      isCoachCaptain(squad, coachName) &&
                      squad.requests.length > 0
                    "
                  >
                ...
                <div v-if="squad.settingsVisible" class="squadsettings">
                  <div class="pendingRequestsList">
                    <div
                      v-for="request in squad.requests"
                      :key="request.team.id"
                      class="squad-request"
                    >
                      ...
                      <div class="request-controls">
                        <button
                          class="accept-btn"
                          @click="acceptRequestMember(request, squad)"
                          :disabled="
                            squad.members.length >= squad.limits.maxMembers
                          "
                        >
                          Accept as member
                        </button>
                        <button
                          class="accept-btn"
                          @click="acceptRequestReserve(request, squad)"
                          :disabled="
                            squad.reserves.length >= squad.limits.maxReserves
                          "
                        >
                          Accept as reserve
                        </button>
                        <button
                          @click="declineRequest(request, squad)"
                          class="decline-btn"
                        >
                          Decline
                        </button>
                  ...
                  <div class="controls" v-if="isCoachCaptain(squad, coachName)">
                    ...
                    <button
                      class="DisbandButton"
                      v-if="isCoachCaptain(squad, coachName)"
                      @click="disbandSquad(squad)"
                    >
                      Disband Squad
                    </button>
                  </div>
              ...
              <div class="pendingrequests">
                ...
                >
                  <div>
                    {{ request.squad.name }}
                    <span class="captain"
                      >(Captain: {{ request.squad.captain }})</span
                    >
                  </div>
  ...
  >
    ...
    <template #body>
      <div class="joinSquadModal">
        <div>
          Select a team to join <b>{{ joinModalSquad?.name }}</b
          >:
        </div>
        <div v-if="userTeams.length === 0">No teams available.</div>
        <div v-else>
          <select v-model="selectedJoinTeam">
            <option v-for="team in userTeams" :key="team.id" :value="team">
              {{ team.name }}
            </option>
          </select>
        </div>
  ...
  <Modal
    ...
    @confirm="confirmRenameSquad"
    @cancel="cancelRenameSquad"
  >
    <template #header>
      ...
      <div class="renameSquadModal">
        <div>
          <input
            class="rounded-input"
            id="renameSquadInput"
            ref="renameSquadInput"
            v-model="renameSquadName"
            type="text"
            minlength="1"
            required
            @keyup.enter="confirmRenameSquad"
...
class TournamentSquads extends Vue {
  ...
  public async loadTeams() {
    this.userTeams = (
      await this.fumbbl.Coach.activeTeams(this.coachName || "")
    ).teams;
  }
  ...
  public async loadRequests() {
    this.pendingRequests =
      await this.fumbbl.TournamentSquads.getPendingRequests();
  }
  ...
  public cancelRequest(request: any) {
    this.fumbbl.TournamentSquads.cancelRequest(
      request.squad.id,
      request.team.id,
    );
    this.pendingRequests = this.pendingRequests.filter(
      (r) => r.squad.id !== request.squad.id || r.team.id !== request.team.id,
    );
  }

  public async searchSquads() {
    this.searchResults = await this.fumbbl.TournamentSquads.search(
  ...
  public requestToJoin(squad: any) {
    ...
    this.selectedJoinTeam =
      this.userTeams.length > 0 ? this.userTeams[0] : null;
  ...
  public sendJoinRequest() {
    ...
    this.fumbbl.TournamentSquads.sendJoinRequest(
      this.joinModalSquad.id,
      this.selectedJoinTeam.id,
    );
  ...
  public async acceptRequestMember(request: any, squad: any) {
    await this.fumbbl.TournamentSquads.acceptRequestMember(
      squad.id,
      request.team.id,
    );
  ...
  public async acceptRequestReserve(request: any, squad: any) {
    await this.fumbbl.TournamentSquads.acceptRequestReserve(
      squad.id,
      request.team.id,
    );
  ...
  public async declineRequest(request: any, squad: any) {
    await this.fumbbl.TournamentSquads.declineRequest(
      squad.id,
      request.team.id,
    );
    squad.requests = squad.requests.filter((r: any) => r !== request);
  ...
  public async onDropMember(
    event: DragEvent,
    squad: any,
    targetIsReserve: boolean,
  ) {
    ...
    const targetElem = (event.target as HTMLElement).closest(
      ".member:not(.empty)",
    );
    if (targetElem) {
      const targetId =
        targetElem.getAttribute("teamid") ||
        targetElem.getAttribute("data-key");
  ...
  public async confirmRenameSquad() {
    ...
    await this.fumbbl.TournamentSquads.rename(
      this.renameModalSquad.id,
      this.renameSquadName.trim(),
    );
